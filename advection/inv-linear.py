...
resultPATH = "pcalin-inv/" #"/localdata/Derick/stuart_data/Darcy_421/inv-linear/"
#Read Datasets
...
for step_size in [2500]:#[2500, 5000, 10000]: #
    for gamma in [0.01]:#[1e-1, 1e-2, 1]: #
        for wd in [0.1]:#[1.5e-2, 1e-1, 1.5e-1]: #
            for batch_size in [500]:#, 1000]:#[100, 200, 500, 1000]: #
                for learning_rate in [0.001]:#[1e-3, 2.5e-3, 1e-2]: #
                    torch.manual_seed(0)
                    np.random.seed(0)
                    ...
                    U_PCALin = pcaY.inverse_transform(inPCALin).flatten()
                    ...
                    plt.ylabel('y')#, fontsize=16, labelpad=15)
                    plt.title("PCALin")
                    plt.plot(U_PCALin)#, cmap=colourMap, extent=[params["xmin"], params["xmax"], params["ymin"], params["ymax"]], origin='lower', aspect = 'auto')#, vmin=0, vmax=1, )
                    #plt.colorbar()#format=OOMFormatter(-5))
                    ...
                    plt.plot(np.abs(U_FDM - U_PCALin))#, cmap=colourMap, extent=[params["xmin"], params["xmax"], params["ymin"], params["ymax"]], origin='lower', aspect = 'auto', norm=matplotlib.colors.LogNorm())#, vmin=0, vmax=1, )
                    #plt.colorbar()#format=OOMFormatter(-5))
                    plt.yscale('log')
