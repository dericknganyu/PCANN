...
wd = 0.1
batch_size = 100
learning_rate = 0.0001000000
p_drop= 0.01
test_l2 = 0.098825
TIMESTAMP = '20221017-235323-975130'
...
if algo == 'adam':    
    ModelInfos = "_UGN_inv_adam_%03d"%(res)+"~res_%.6f"%(test_l2)+"~RelL2TestError_"+str(dX)+"~rd_"+str(p_drop)+"~pdrop_"+str(ntrain)+"~ntrain_"+str(ntest)+"~ntest_"+str(batch_size)+"~BatchSize_%.10f"%(learning_rate)+\
...
plt.xlabel('x')#, fontsize=16, labelpad=15)
plt.ylabel('y')#, fontsize=16, labelpad=15)
plt.title("Truth-PCANN, RelL2Err = "+str(round(myLoss.rel_single(U_PCANN, U_FDM).item(), 3)))
...
fig.tight_layout()
plt.savefig(resultPATH+'figures/PCANN-DarcyPWC-Inverse-UP>png',dpi=500)
