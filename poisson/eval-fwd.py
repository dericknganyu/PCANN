...
resultPATH = "pcann/"
modelPATH  = "/localdata/Derick/stuart_data/Darcy_421/poisson/--fwd2/RN/epochs 20000 step 2000 gamma 0.5/"
#Read Datasets
...
p_drop= 0
test_l2 = 0.007366
TIMESTAMP = '20221011-181648-522053'
...
if algo == 'adam':    
    ModelInfos = "_adam_%03d"%(res)+"~res_%.6f"%(test_l2)+"~RelL2TestError_"+str(dX)+"~rd_"+str(p_drop)+"~pdrop_"+str(ntrain)+"~ntrain_"+str(ntest)+"~ntest_"+str(batch_size)+"~BatchSize_%.9f"%(learning_rate)+\
            "~LR_%.4f"%(wd)+"~Reg_%.4f"%(gamma)+"~gamma_%s"%(step_size)+"~Step_"+str(epochs)+"~epochs_"+TIMESTAMP
...
fig.tight_layout()
plt.savefig('figures/PCANN-Poisson-Forward-UG.png',dpi=500)
