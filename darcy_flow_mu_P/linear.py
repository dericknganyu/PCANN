...
resultPATH = "pcalin/"#"/localdata/Derick/stuart_data/Darcy_421/darcy_flow_mu_P/-fwd-linear/"
#Read Datasets
...
for step_size in [10000]:#2500, 5000, 10000]:
    for gamma in [1e-2]:#1e-1, 1e-2, 1]:
        for wd in [0.1]:#0, 1e-3, 1.5e-2, 1e-1, 1.5e-1]: #
            for batch_size in [1000]:#100, 200, 500, 1000]:
                for learning_rate in [1e-3]:#, 2.5e-3, 1e-2]:
                    torch.manual_seed(0)
                    np.random.seed(0)
                    ...
                    ################################################################
                    model = pcann(params).cuda()#pcann(modes, modes, width).cuda()
                    ...
                    if algo == 'adam':    
                        ModelInfos = "_adam_%03d"%(res)+"~res_"+str(np.round(test_l2,6))+"~RelL2TestError_"+str(dX)+"~rd_"+str(ntrain)+"~ntrain_"+str(ntest)+"~ntest_"+str(batch_size)+"~BatchSize_"+str(learning_rate)+\
                                "~LR_"+str(wd)+"~Reg_"+str(gamma)+"~gamma_"+str(step_size)+"~Step_"+str(epochs)+"~epochs_"+datetime.utcnow().strftime('%Y%m%d-%H%M%S-%f') 
                    
                    if algo == 'sgd':
                        ModelInfos = "_sgd_%03d"%(res)+"~res_"+str(np.round(test_l2,6))+"~RelL2TestError_"+str(dX)+"~rd_"+str(ntrain)+"~ntrain_"+str(ntest)+"~ntest_"+str(batch_size)+"~BatchSize_"+str(learning_rate)+\
                                "~LR_"+str(wd)+"~Reg_"+str(gamma)+"~gamma_"+str(step_size)+"~Step_"+str(epochs)+"~epochs_"+datetime.utcnow().strftime('%Y%m%d-%H%M%S-%f') 
                    ...
                    plt.xlabel('x')#, fontsize=16, labelpad=15)
                    plt.ylabel('y')#, fontsize=16, labelpad=15)
                    plt.title("FDM-PCALin, RelL2Err = "+str(round(myLoss.rel_single(U_PCANN, U_FDM).item(), 3)))
