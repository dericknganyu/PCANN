...
for res in resolution:
    reduced_dim = [100, 150, 70, 30]#50, 70, 150, 200] #[50, 70, 100, 150, 200, 250] #[70, 50]#[30, 50, 70, 100, 150, 200, 250] #[30, 50, 70, 100]#
    gpu_infos   = [4  , 5  , 6  , 7  ] #[0  , 1  ,   2,   3]#, 1 , 2  , 3  ] #[4 , 5 , 6  , 7  ] #[0 , 1 , 2  , 3  ] #[0, 7]#[1 , 2 , 3 , 4  , 5  , 6  , 7  ] #[0, 1 , 2 , 3] #[1 , 2 , 3 , 4  , 5  , 6  , 7  ] #

    parser = argparse.ArgumentParser(description='parse mode')
    parser.add_argument('--mode' , default ='inv-lin', type = str, help='fwd for forward, inv for inverse')#
    ...
    #parser.add_argument('--res' , default='32'   , type = int, help='resolutions factors of 512')#

    args = parser.parse_args()

    if args.mode == 'fwd':
        name_screen    = 'pcann_darcyPWC_algo2_%s_res_%s_rd_'%(args.algo, res)
        command_screen = 'python pcann-snn2.py --algo %s --res %s --dX '%(args.algo, res)

    if args.mode == 'inv':
        name_screen    = 'inv-pcann_darcyPWC_algo21_%s_res_%s_rd_'%(args.algo, res)
        command_screen = 'python inv-pcann-snn2.py --algo %s --res %s --dX '%(args.algo, res)

    if args.mode == 'fwd-lin':
        name_screen    = 'linear_darcyPWC_algo3_%s_res_%s_rd_'%(args.algo, res)
        command_screen = 'python linear.py --algo %s --res %s --dX '%(args.algo, res)

    if args.mode == 'inv-lin':
        name_screen    = 'inv-linear_darcyPWC_algo_%s_res_%s_rd_'%(args.algo, res)
